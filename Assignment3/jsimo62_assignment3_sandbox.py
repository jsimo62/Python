...
def print_to_console(response, w_avg_cm_all, w_avg_hm_all, c_vehicles, m_vehicles):
    if response == 'all':
        print('\n***AVERAGE MPG FOR ALL***')
        print(w_avg_cm_all)
        print(w_avg_hm_all)
    ...
    return
def output_to_file(response, w_avg_cm_all, w_avg_hm_all, c_vehicles, m_vehicles):
    # 1 - Create/Open File
...
while res_attempt < 4:
    ...
    if response == "all":
        ...
        print_to_console(response, w_avg_cm_all, w_avg_hm_all, {}, {})
        output_to_file(response, w_avg_cm_all, w_avg_hm_all, {}, {})
        res_attempt = 5
    elif response == "group":
        scan_file(file)
        print_to_console(response, [], [], c_vehicles, m_vehicles)
        output_to_file(response, '', '', c_vehicles, m_vehicles)
        res_attempt = 5
    else:
        res_attempt += 1
        if res_attempt <= 2:
            ...
            print("\nThis is your last attempt.  Please type either 'all or 'group' to continue.\n**The program will end after 1 more inncorrect entry**\n")
        else:
            print("\nGood Bye")
